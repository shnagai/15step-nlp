...
def tokens_to_sequence(we_model, tokens):  # <3>
    sequence = []
    for token in tokens:
        try:
...
def get_keras_embedding(keyed_vectors, *args, **kwargs):  # <6>
...
if __name__ == '__main__':
    ...
    tokenized_training_texts = [tokenize(text)
                                for text in training_texts]  # <2>
    training_sequences = [tokens_to_sequence(we_model, tokens)  # <3>
                          for tokens in tokenized_training_texts]

    # training_sequencesの全要素の長さをMAX_SEQUENCE_LENGTHに揃える
    MAX_SEQUENCE_LENGTH = 20
    x_train = pad_sequences(training_sequences,
                            maxlen=MAX_SEQUENCE_LENGTH)  # <4>
    ...
    # 学習
    model.fit(x_train, to_categorical(y_train), epochs=50)  # <7>
